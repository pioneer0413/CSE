...
class SpikeTemporalBlock2D(nn.Module):
    ...
    def forward(self, x):
        ...
        #print(f'[TemporalBlock2D] spike count conv1: {spks1.sum()}({spks1.numel()})')
        ...
        #print(f'[TemporalBlock2D] spike count conv2: {spks2.sum()}({spks2.numel()})')
        ...
        #print(f'[TemporalBlock2D] spike count lif: {res.sum()}({res.numel()})')
        res = res.mean(-1)

        return res


@NETWORKS.register_module("SpikeTCN")
